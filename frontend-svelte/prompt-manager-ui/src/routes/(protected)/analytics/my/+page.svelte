  ...
  onMount(async () => {
    ...
    try {
      const [v, f, c] = await Promise.all([
        ...
        apiFetch(`${analyticsUrl}/most-favorited`).then((r) => r.json()),
...
{#if loading}
  <div class="text-center text-gray-500 mt-8">Loading your analytics…</div>
{:else if unauthorized}
  <div class="text-center text-red-500 mt-8">Access denied or no data.</div>
{:else}
  <section class="p-8 space-y-10">
    <h1 class="text-2xl font-bold text-center">My Prompt Analytics</h1>

    <!-- Most Viewed -->
    <div class="bg-white shadow-sm rounded-xl p-4 space-y-3">
      <h2 class="font-semibold mb-2 text-lg">Most Viewed</h2>
      {#if viewed.length === 0}
            ...
            >
              <div>
                ...
                <div class="text-xs text-gray-500 line-clamp-2 mt-0.5">
                  {item.prompt?.description ?? "No description"}
                </div>
                <div
    ...
    <!-- Most Favorited -->
    <div class="bg-white shadow-sm rounded-xl p-4 space-y-3">
      <h2 class="font-semibold mb-2 text-lg">Most Favorited</h2>
      {#if favorited.length === 0}
        <p class="text-gray-500 text-sm text-center py-3">No favorites yet.</p>
      {:else}
        <div
          class="flex justify-between items-center text-xs uppercase tracking-wide text-gray-500 mb-1 px-1"
        >
    ...
    <!-- Most Copied -->
    <div class="bg-white shadow-sm rounded-xl p-4 space-y-3">
      <h2 class="font-semibold mb-2 text-lg">Most Copied</h2>
      {#if copied.length === 0}
        <p class="text-gray-500 text-sm text-center py-3">
